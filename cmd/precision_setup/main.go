...
var (
	config = PrecisionConfig{
		...
		SpellTargets: make(map[string]image.Point),
	}
...
func main() {
	...
	win.SetMouseHandler(func(event int, x, y int, flags int, userdata interface{}) {
		if event == 1 { // LBUTTONDOWN
			...
			case 7: // Spell Target Spot
				config.SpellTargets["BottomLeft"] = p
				fmt.Printf("✓ Spell Target Spot set to %v\n", p)
			case 8: // Village Center
				centerPt = p
	...
	for {
		...
		for _, p := range config.SpellTargets {
			gocv.Circle(&display, p, 10, color.RGBA{255, 0, 255, 255}, 2)
		}
		...
		if key == 'q' {
			...
			config.SpellTargets = make(map[string]image.Point)
			config.BarY = 0
			tempPoints = nil
			centerPt = image.Point{}
			...
			case 8:
				...
				for _, name := range []string{"BottomRight", "TopLeft", "TopRight"} {
					...
					delete(config.SpellTargets, name)
					delete(config.SpellEdgesA, name)
					delete(config.SpellEdgesB, name)
				}
			case 7:
				delete(config.SpellTargets, "BottomLeft")
			case 0, 3, 5:
				if len(tempPoints) > 0 {
...
func mirrorPlacements() {
	...
	blSpellTarget := config.SpellTargets["BottomLeft"]
	...
	config.SpellTargets["BottomRight"] = image.Pt(2*Cx-blSpellTarget.X, blSpellTarget.Y)
	...
	config.SpellTargets["TopLeft"] = image.Pt(blSpellTarget.X, 2*Cy-blSpellTarget.Y)
	...
	config.SpellTargets["TopRight"] = image.Pt(2*Cx-blSpellTarget.X, 2*Cy-blSpellTarget.Y)
}
