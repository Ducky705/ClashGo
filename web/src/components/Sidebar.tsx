...
const Sidebar: React.FC<SidebarProps> = React.memo(({
  ...
  return (
    <aside 
    className={`fixed left-0 top-0 h-full bg-white dark:bg-zinc-900 border-r border-zinc-100/50 dark:border-zinc-800/50 z-50 transition-[width,background-color] duration-200 ease-out group ${expanded ? 'w-64 shadow-premium-lg dark:shadow-none' : 'w-20'}`}
    style={{ 
      position: 'fixed', 
      left: 0, 
      top: 0, 
      ...
      <nav className="space-y-1.5 flex-grow">
        {menuItems.map((item) => (
          ...
          >
            ...
            <span className={`text-[13px] font-bold tracking-tight transition-[opacity,transform] duration-200 ease-out whitespace-nowrap ${expanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-2'}`}>
              {item.label}
            </span>
          </button>
        ))}
