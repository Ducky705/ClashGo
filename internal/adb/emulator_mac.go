...
func (c *Client) EnsureBlueStacksMac(width, height, dpi int) error {
	...
	if err := c.waitForVMProcess(45 * time.Second); err != nil {
		// 45s after launch, qemu-system-aarch64 / hd-adb never appeared.
		...
		return fmt.Errorf("BlueStacks VM subsystem did not start after 45s (qemu-system-aarch64 / hd-adb never appeared). Open the BlueStacks Air multi-instance manager, select Tiramisu64, click Start, then click Retry in this app: %w", err)
	}
	if err := c.waitForBlueStacksADB(60 * time.Second); err != nil {
...
// on first hit. 60s is the budget because BlueStacks 5.21 on a slow
// Mac can take 30-45s to fully spawn qemu-system-aarch64 after
// `open -a BlueStacks.app`.
//
...
func (c *Client) waitForVMProcess(timeout time.Duration) error {
...
func (c *Client) waitForBlueStacksADB(timeout time.Duration) error {
	if c.DeviceID == "" {
		return nil
	}
