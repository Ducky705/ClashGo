...
// UpgradeWalls executes the automated wall upgrade sequence.
func (b *Bot) UpgradeWalls(gc *game.GameContext) {
	...
	if !ok {
		b.logger.Warn().Msg("Wall upgrade skipped: not in main village")
		return
	}
	...
	// Swipe strictly within the scrollable container bounds to avoid borders/headers
	topMargin := int(80 * b.cal.ScaleY)
	bottomMargin := int(60 * b.cal.ScaleY)
	sy1 := menuROI.Max.Y - bottomMargin
	sy2 := menuROI.Min.Y + topMargin

	// 3. Scroll to the dead bottom of the menu (swipe Y from bottom to top)
	b.logger.Debug().Int("scrollX", scrollX).Int("sy1", sy1).Int("sy2", sy2).Msg("Scrolling upgrades menu to the bottom")
	for i := 0; i < 3; i++ {
		if err := b.client.Swipe(scrollX, sy1, scrollX, sy2, 500); err != nil {
	...
	wallClicked := false
	scrollUpY1 := menuROI.Min.Y + topMargin
	scrollUpY2 := menuROI.Max.Y - bottomMargin
	...
	for attempt := 0; attempt < 6; attempt++ {
		...
		// Search for Wall text with a robust threshold of 0.60 and 60 scale steps
		matches, _ := vision.MatchMultiScaleROI(screen, wallTpl, 0.3, 1.5, 60, 0.60, menuROI)
		screen.Close()
		...
		// Scroll up slowly to show items above
		b.logger.Debug().Int("scrollX", scrollX).Msg("Wall text not visible, scrolling up slowly...")
		if err := b.client.Swipe(scrollX, scrollUpY1, scrollX, scrollUpY2, 500); err != nil {
	...
	if !wallClicked {
		b.logger.Warn().Msg("Failed to locate Wall text in builder menu")
		b.client.Back() // Close menu
		return
	}

	// 5. Wait for map camera to focus on the selected wall and upgrade menu to show
	...
	for idx, match := range uniqueMatches {
		...
		time.Sleep(1000 * time.Millisecond) // Wait for confirm dialog or gem purchase popup
		...
		if len(confirmMatches) > 0 {
			...
			b.client.Back()
			time.Sleep(800 * time.Millisecond) // Wait for dialog to close
		}
	}

	if success {
		b.logger.Info().Msg("Wall upgrade completed successfully!")
	} else {
		b.logger.Warn().Msg("Failed to upgrade wall: all options checked, none were affordable")
		b.dismissSelection()
	}
}
